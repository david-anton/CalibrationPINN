...
from calibrationpinn.data.simulation_2d import SimulationDataset2D
from calibrationpinn.data.trainingdata_2d import SimplifiedDogBoneTrainingDataset2D
from calibrationpinn.io import ProjectDirectory
from calibrationpinn.io.loaderssavers import PytorchModelSaver
...
from calibrationpinn.training.metrics import mean_absolute_error, relative_l2_norm
...
class TrainingConfiguration:
    ...
    training_dataset_pinn: TrainingDatasetDogboneLike2D
...
def train_parametric_pinn(train_config: TrainingConfiguration) -> None:
    ...
    def loss_func(
        ...
        #     x_param = symmetry_bc_data.x_params.to(device)
    ...
    def loss_func_closure() -> float:
        optimizer.zero_grad()
        loss_collocation, loss_traction_bc, loss_data = loss_func(
            ansatz, batch_collocation, batch_traction_bc, batch_data
        )
        loss = loss_collocation + loss_traction_bc + loss_data
    ...
    for epoch in range(train_num_epochs):
        ...
        for batch_collocation, batch_traction_bc, _ in train_batches_pinn:
            ansatz.train()
