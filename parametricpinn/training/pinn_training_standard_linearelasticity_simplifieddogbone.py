...
class TrainingConfiguration:
    ...
    weight_regularization_loss: float
...
def train_parametric_pinn(train_config: TrainingConfiguration) -> None:
    ...
    weight_regularization_loss = train_config.weight_regularization_loss
    ...
    def loss_func(
        ...
        def loss_func_symmetry(
            ...
            def _loss_func_dirichlet_bc() -> Tensor:
                slice_top = slice(num_points_per_bc, 3 * num_points_per_bc)
                x_coor_top = traction_bc_data.x_coor[slice_top, :].to(device)
                x_param_top = traction_bc_data.x_params[slice_top, :].to(device)
                x_top = torch.concat((x_coor_top, x_param_top), dim=1).to(device)
                y_top = ansatz(x_top)
                y_top_x = y_top[:, 0]
                y_top_y = y_top[:, 1]

                slice_bottom = slice(3 * num_points_per_bc, 5 * num_points_per_bc)
                x_coor_bottom = traction_bc_data.x_coor[slice_bottom, :].to(device)
                x_param_bottom = traction_bc_data.x_params[slice_bottom, :].to(device)
                x_bottom = torch.concat((x_coor_bottom, x_param_bottom), dim=1).to(
                    device
                )
                y_bottom = ansatz(x_bottom)
                y_bottom_x = y_bottom[:, 0]
                y_bottom_y = y_bottom[:, 1]

                y_x = y_top_x - y_bottom_x
                y_y = y_top_y + y_bottom_y
                y = torch.concat((y_x, y_y), dim=0)
                y_true = torch.zeros_like(y, device=device)
                return loss_metric(y_true, y)

            loss_dirichlet = _loss_func_dirichlet_bc()
            return loss_dirichlet

        def loss_func_regularization(
            ansatz: StandardAnsatz, collocation_data: TrainingData2DCollocation
        ) -> Tensor:
            x_coor = collocation_data.x_coor.to(device)
            x_param = collocation_data.x_params.to(device)
    ...
    for epoch in range(train_num_epochs):
        ...
        loss_hist_regularization_batches = []
        ...
        mean_loss_regularization = statistics.mean(loss_hist_regularization_batches)
        ...
        loss_hist_regularization.append(mean_loss_regularization)
        ...
        print(f"REGULARIZATION: \t {mean_loss_regularization}")
        print("##################################################")
